...
`FusionAhrsUpdateNoMagnetometer` depends on `m_magcal.FHasSolution()`, which is private to
...
- `libcalib::MagCalibrator` — magnetometer sphere fitting and quality metrics;
...
- `m_magcal.FHasSolution()` is the correct way to test whether a valid magnetometer
