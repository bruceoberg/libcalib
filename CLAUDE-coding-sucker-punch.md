...
How easy is it to understand and debug existing code?
How easy is it to use existing code in the correct way?
How easy is it to write new code which performs correctly?
How small are the performance penalties (particularly for the engine).
...
Method and function names should be formatted with [Optional Return Type]VerbNoun. If the function returns void, skip the return type.
...
Non-existent for local scope
g_ for global scope
s_ for static scope. Note that TWEAKABLE variables have static scope, and should use the s_ prefix.  Header file 
m_ for class/struct scope

Prefix values

p for pointer.
ea for a pointer to main memory, for code which runs on the SPU and needs to distinguish between local memory addresses and main memory addresses.
c for count
i for index
r for ratio
u for unsigned. Note that u is also a tag, used for "parameter"-style floats with values between 0 and 1. In addition, u is the tag for the first part of a 2D texture coordinate.
d for delta or change in value. This should be used for a measured or expected change, and not for a rate of change. So, "dcFoo" means "the count of foos changed by this much", not "this much change in count of foos, per second". For the latter, you would add information to the detail of the name and get (something like) "cFooPerSecond".
a for a C-style array
ary for templated array
circ for a circular buffer
dl for a doubly-linked list
sl for a singly-linked list
mp for an array which "maps" one type to another; for instance, mpFooPChz is an array which maps the enum type Foo to a string. Note the capitalization in this case. 
Note that mp can be used with C-style arrays, and also with templated arrays.
Capitalization should be upper case for the first letter of the 2 tags and normal for everything else (e.g. mpTag1Tag2).
hash for a hash table. Ex. CHash<CString, OID> hashStrOid;
set for a set.
Note that if a variable name starts with multiple prefix values, values beyond the first should not have their first letter capitalized. For example, an array of arrays should be named aryaryFoo, and an array of pointers should be named arypFoo.

There are also some prefixes which only really make sense in a particular context. As they become known, they should be documented here:

s for a directionless velocity. Typically, sV = Length(v).
dV for delta velocity (although we try to use the more specific tag dv to mean acceleration (derivative of velocity))

Tag values
Shortened version of a class, or the class name itself if the class name is short.
First letter capitalized if there is a prefix value.

Some standard tags:

b for byte (u8)
ch for character (char).
g for float
u for float between 0 and 1
su for float between -1 and 1
s for distance
f for boolean
t for time
n for integer. You shouldn't see many integers in the code; generally, there's a better Hungarian description than just integer.
deg for degrees
rad for radians
pos for point
vec for vector. Note the distinction between a position (= Point) and a direction (= Vector), which is enforced by our math libraries.
x, y or z for a single coordinate value on each axis. Combinations can be used for a single tag (e.g. Xy or Xyz).
normal - sometimes a tag for normalized vectors, and sometimes a tag for normal vectors.
mat for matrix or transform.
Use matWorldToClip (AToB) to signify what space matrices are transforming where possible. This way space transitions are much clearer. E.g. posObjB = posObjA * matObjAToWorld * matWorldToObjB.
v for velocity.  Prefixes can be used to clarify use of velocity: sV (magnitude of velocity).
By convention, we use dv for acceleration (like gravity) vs dV for instantaneous change in velocity
w for angular velocity.  Similar rules as apply to v (e.g. dw and sW are angular acceleration/torque and magnitude of angular velocity).
eul for a set of euler angles in radians.
For enum types, the name of the enum, all lowercase. TRAIT becomes trait, FBTN fbtn, GRFBTN grfbtn, and INSTANCEK instancek.
Less standard tags:

mp for the "juice" that Cole and many electric objects store (aka Magic Points).
hp for Hit Points of a character.
dw for DWORD

Notes:

Use dX, dY, dZ or similar for texel, pixel, voxel dimensions.
In the rare case that a type follows a full tag, you should generally capitalize the first letter of the 2nd type like you would for a mp (e.g. lmDT).
Details
Whatever is needed for specifics of variable
Suffix
Min: Minimum value, or minimum index
Max: Maximum value, or maximum index (last valid index + 1). i.e. Max is not a valid index - an array with 3 elements would have Max = 3 (2 being the highest valid index).
Mic: The same as Min, but for a subrange of a larger list. (Mic >= Min of the container)
Mac: The same as Max, but for a subrange within a a larger list. (Mac <= Max of the container)
In short - if you're working with a subrange of a larger container, use Mic and Mac for its bounds so it doesn't get confused with the Min and Max of the container.
First: An alternative to using Min (First = Min). Useful when using Last.
...
If youâ€™re looping, but not looping over anything, then you can use a naked â€œiâ€ as the iteration variable. Similarly, an interation count can be a naked â€œcâ€.
...
Enums
Do not use enum classes.
Disallowing integer conversion breaks common enum table use like mpEnumToPChz.

Name the enumerated type in all caps.

Name the values using descriptive names prefixed by the enum name and an underscore (like ENUM_ValueName).
For bit-flags enums use F as the *first* letter of the enumeration (like FENUM).
Consider using DEFINE_ENUM_FLAGS to define helpful functions.
To indicate that a particular instance of a flags enum may contain multiple bits, prefix with GRF (like GRFENUM).
Use descriptive names otherwise.
...
Consider using DEFINE_ENUM_INCDEC to define helpful functions.
Consider using the DEFINE_ENUM_INCDEC or DEFINE_ENUM_FLAGS to create helpful operators.
For enums inside classes or namespaces use the DECLARE and IMPLEMENT_*_OPERATORS macros.
Manually specified an integral type if you need the enum to be bigger or smaller than an int.
...
Templates
In general, follow the standard rules for naming. Template Classes should be named the same as concrete classes, Template Functions should follow the same names as concrete functions, etc.
...
Structs, classes and namespaces are â€œcolumnifiedâ€. Nothing else should be. Even for structs et al, donâ€™t be a slave to the columnsâ€¦ different sections of the struct, class or namespace can use different column arrangements.
Really long type names are allowed to spill over columns boundaries. If they do, wrap down to the next line and line the variable in the right column.
...
Follow the following guidelines when using namespaces:

Use to collect related groups TWEAKABLE options for a class or system.
The namespace uses the name of the associated class, minus the leading â€œCâ€. If the namespace is not directly associated with a classâ€”which should be unusualâ€”then choose a reasonable name.
Typically, the namespace is confined to the C++ class. It immediately precedes the method implementations for the class.

We follow class-style formatting conventions for namespaces, with members and comments lined up in pretty columns.
...
// If an initializer needs to be split on multiple lines, it'll follow our standard of tabbing in once from the function name:
...
// Dealing with preprocessor removing all initializers (since we can't have a trailing : with no initializers)
...
The DASSERT macro only compiles in the debug build. It is useful for expensive checks and validation, but should be used sparingly. Remember that very few people will be running the debug build, so most people won't evaluate the assert.
...
Use ASSERTs at the start of a function to make sure that the parameters passed in are correct.
If you provide an auditing function on the class which checks it for validity, then it should be named FIsValid. If the auditing function does not have a return value, but instead asserts for validity inside the function, then the auditing function should be named Audit.
...
Descriptive comments for functions arenâ€™t required, but they are suggested.
Donâ€™t waste time making beautifully formatted banners.
Donâ€™t worry about Doxygen or other document generating extractors. Use the code base as documentation. Save a tree.
All members of structs/classes should be commented, except the trivially obvious ones.

Header files

Provide a one-to-two-line description of the purpose of the class. If you canâ€™t make it short, it probably means the class may need to be refactored.
...
Donâ€™t comment local variables.
Comments shouldnâ€™t just repeat the line of code below it.
Use comments to explain the block of code which follows. This helps separate straight-line sections, too.
Provide URLs to places describing the algorithm you are using. VisualStudio will follow URLs in comments.

Spacing

Here are the following rules for spacing:

3 newlines between #includes and code.
...
Making judicious use of access control on classes helps expose the intended use of the class through the public interface. It also helps limit incorrect usage due to compiler errors. It does make writing the class a little more difficult.
...
In general, don't be afraid to use the friend mechanism. Access control is supposed to be helping you write code, not hurting. If you have a pair, or small set, of classes that work together, it's fine to friend them to each other. We do this with our data-instance split for CLo, for example.
...
Place automatically generated functions in the protected section if you donâ€™t want clients to use them. This includes the default constructor, the copy constructor, the destructor, and the assignment operator.
If you really want to prohibit somethingâ€”say, copy constructionâ€”then you can add the method to a private section of the class. This should be extremely rare.
Lay out the class declaration so the public section comes first, followed by the protected section. Generally, you should have a single public section, followed by a single protected section, as this makes your class declaration easier to follow.
...
For â€œbushyâ€ class hierarchies like the CBasic tree, use extremely simple constructors and destructors, and do more work in Init and Destroy methods.
...
Donâ€™t try to handle double destruction.
Use initializer lists rather than code in the body of the constructor to initialize member variables when possible.
Initializing arrays of constructor-less types via constructor syntax is accepted practiceâ€¦ however, note that this will zero-fill the array, which may not be advisable.
Members should be listed in the order in which they appear in the class declaration.
Donâ€™t do virtual dispatch within constructors or destructors. The rules for how this works are funky, and generally people don't get it right. If you need to do virtual dispatch during the construction or destruction process, you should split out a separate method (like Init, or Unload) and make your virtual function calls there.
Avoid memset, memcpy, memmove and other bit-clobbering routines unless you know what youâ€™re doing.


Const

Strive for const correctness. The primary benefit is to document your intent. A secondary benefit is to let the compiler prevent inappropriate use. Although it may seem like the compiler could generate better performing code with const, this turns out to not be the case in general (see http://www.gotw.ca/gotw/081.htm).
...
Donâ€™t change semantics if you override a method simply on constness.
...
public:
    ...
    void MutateSelf(); // donâ€™t make const
...
public:
    ...
    void DoSomething() const
        { printf(â€œHappy happy joy joyâ€); }
...
However, there are definitely times where it is convenient or even necessary to cast.

In those cases, use C++ style casting instead of C-style casting. The main benefit is to be clear with your intent.

Use reinterpret_cast where appropriate.
Be careful of accessing the same memory between reinterpret_casted types.  Compilers often optimize load/store code based on types that "cannot alias" (aka point to the same memory).  E.g. writing to a float * and then reading from the same address as an int * can cause problems.  Using unions can convince the compiler to perform the loads and stores in the correct order.
Use derived_cast on Basic derived classes (which use the RTI macro).
Use rti_cast on Basic derived classes if the class may not be the correct class (combines checking FIsDerivedFrom and derived_cast).
Use static_cast in other situations.
Do not use dynamic_cast, since we donâ€™t support RTTI.
...
When the compiler requires a cast, constructor-style casting is encouraged for simple types. Examples are converting and int to an enum or converting a Scalar to a float.
Examples:
...
Make sure you donâ€™t create cycles of smart pointers, this will cause memory to never be freed.
...
Donâ€™t use macros to #define constants.
Donâ€™t use macro functions where a template or inline function would do.
...
Use of the ternary operator (?:) is allowed for cases in which the conditional and arguments are very simple. Paretheses are required around the conditional part (before the ?). If the entire expression doesn't fit on a single line, write it on three lines.
...
You can use booleans as function arguments.  In some cases raw values can be confusing though.  Flag enums for multiple options, two value enums in place of true/false, or commenting the name of argument are ways to aid readability.
...
References are encouraged, but only as an optimization for pass-by-value. This means all references should be a const reference.
One exception is passing writable arrays to functions. There you should pass non-const array & (float (& aG)[3]) instead of c array (float aG[3]) since the former is type safe based on size and the later turns into a pointer and does not check the size.
Use pointers for objects such as CLoâ€™s, even if they are guaranteed non-NULL and even if they will only have const functions called on them, since pass-by-value makes little sense for them.
...
The key word here is â€œlocalâ€. Make the variables have as short a lifetime as possible, which will reduce the amount of data people need to keep in their head when re-reading the code.
...
Use each local variable for one purpose only. Donâ€™t shy away from creating new local variables for different meanings.
...
Long lines should be broken once theyâ€™re past 120 characters.
...
Some guidelines for using CString:
...
Use StrFromLit to make literal CStrings.
For SData classes you can dump CDataString or CDebugString using WriteString or WriteDebugString on CBinaryOutputStream in AC. Take care storing these else where in the engine though as the data can be unloaded.
...
auto is only allowed for range-based with const auto & (or with auto * or similar) and, rarely, for template iterator types (see STL disallowed below).
Consider readability when using auto. In some cases (e.g. enums, built-in types, and possibly pointers to classes) it is shorter or clearer to specify the type.
default and delete class specifiers are allowed.  Use delete instead of private unimplemented functions.
initializer_lists are allowed for statically scoped lists.
lambdas can be used in place of functors (e.g. as an argument to a sort function).
Don't use default capture syntax ([&] and [=]) unless it's in debug code and performance is not a consideration.
Consider readability when using lambdas to compress duplicate code involving state.
...
decltype is allowed for use.  As this is generally this is in niche template cases, take care the code is not being over complicated.
...
For constant definition, prefer static const to constexpr where equivalent unless constexpr is required (e.g. CASSERTs).
...
This is a quick list of C++ features not covered in the rest of the document. They are simply broken into which are allowed, which are frowned upon, and which are disallowed.

Allowed

Templates, within reason
operator overloading. Follow the syntax and semantic conventions here.
Nested Classes
Namespaces
